...
*/
    ...
    for key_val in key_val_vec {
        let property_value_builder = PropertyValueBuilder::new();
        if key_val.len() != 9 {
            "Properties are formated incorrectly".to_string();            
        }
        let name = match key_val.get(0) {
            Some(value) => {
                property_value_builder.with_name(value.to_string());
                value.to_string()
            },
        ...
        property_value_builder.with_data_type(data_type.unwrap());
        ...
        if data_type.unwrap() == DataType::String {
            let string_value = match key_val.get(5) {
                Some(value) => {
                    property_value_builder.with_string_value(value.to_string());
                    value.to_string()
                },
                None => "description is formated incorrectly".to_string()
            };    
        ...
        let property_value = property_value_builder.build()
        .unwrap();
